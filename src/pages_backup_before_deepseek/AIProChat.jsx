...
const AIProChat = ({ targetLang, setTargetLang }) => {
  ...
  const T = usePageTranslation(EN, targetLang);
  const t = (key) => T[key] || EN[key];

  // ====== Translation base for CHAT UI ======
  const CHAT_EN = useMemo(
    () => ({
  ...
  const ChatT = usePageTranslation(CHAT_EN, targetLang);
  const ct = (key) => ChatT[key] || CHAT_EN[key] || key;
  ...
  try {
    // Call YOUR backend server (not DeepSeek directly)
...
useEffect(() => {
                  ...
                  <div className="input-area">
                    <div className="input-wrap">
                      <textarea 
                        ...
                        rows={5} 
                        onKeyDown={(e) => {
  // Enter WITHOUT Shift should create new line (do nothing special)
  ...
  if (e.key === 'Enter' && e.shiftKey) {
    return;
                ...
                // Invite Friends UI (without About Us)
